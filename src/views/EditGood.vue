  ...
  <div>
   ...
   <el-card>
      ...
      <el-tab-pane label="商品内容" name="4">
        ...
        <el-button type="primary" class="btnAdd" @click="addGood">编辑商品</el-button>
      </el-tab-pane>
    </el-tabs>
   </el-form>
   </el-card>
...
export default {
  ...
  created () {
    this.goodId = this.$route.query.goodsId * 1
    this.getCateList()
    this.getthisGood()
  },
  ...
  methods: {
    ...
    async getthisGood () {
      const { data: res } = await this.$http.get(`goods/${this.goodId * 1}`)
    ...
    beforeTabLeave (activeName, oldActiveName) {
      if (oldActiveName === '0') {
        if (this.ruleForm.goods_cat.length !== 3) {
          ...
          return false
        } else if (this.ruleForm.goods_price.trim() === '') {
          this.$message.error('请输入商品价格')
          return false
        } else if (this.ruleForm.goods_weight.trim() === '') {
          this.$message.error('请输入商品重量')
          return false
        } else if (this.ruleForm.goods_number.trim() === '') {
    ...
    addGood () {
      ...
      this.$refs.ruleForm.validate(async valid => {
        ...
        const { data: res } = await this.$http.put(`goods/${this.goodId}`, coneform)
